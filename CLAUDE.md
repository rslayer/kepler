...
- The keep threshold is twice the child's own seed spread, so a change that adds seed noise raises its own bar: 80 origins, raw lags 1-3 and num_leaves 31 each showed a real aggregate gain and each missed by under 0.0004 [r038, r043, r051]. num_leaves 31 is the strongest hyperparameter signal on v1 (December folds -0.004/-0.008) while min_child_samples 200 is neutral [r041, r045, r046]; retest leaves only in a lower-noise form, and read the spread before the gain.
...
- Long history: 80 origins beat 40 on the December folds in four v0 runs and one v1 run, and on v1 it doubles the seed spread and loses folds 6-8 [r008, r011, r013, r024; r038]. Retry only with wider spacing or a recency half-life (ledger H022, H023), never bare.
...
- Single-seed and single-fold gains are noise on this data and only the harness verdict counts [adversary r010, r022; r038, r043, r051]; a gain confined to exactly the folds whose windows contain a known calendar event, with zero delta elsewhere, is structural, not luck, and a human can overrule item 5 on that basis [r037, r055].
